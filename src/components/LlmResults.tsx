...
export default function LlmResults() {
  ...
  const uploadResults = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setRows(loaded); setTargetId(""); setMessage(`Loaded ${loaded.length} experiment calls for ${new Set(loaded.map((row) => row.target_behaviour_id)).size} held-out behaviors.`);
  ...
  return <main className="page-shell reward-test-page llm-results-page">
    ...
    <section className="controls-card admin-panel reward-controls"><div className="toolbar admin-upload-row"><label className="field field-wide"><span>Completed experiment results</span><input type="file" accept=".jsonl,.json,application/json" onChange={uploadResults} /></label><label className="field field-wide"><span>Cleaned Prolific data, for top-three replays</span><input type="file" accept=".json,application/json" onChange={uploadReferences} /></label></div>{rows.length ? <div className="toolbar admin-upload-row"><label className="field field-wide"><span>Participant behavior description</span><select value={targetId} onChange={(event) => setTargetId(event.target.value)}>{targets.map((target) => <option key={target.id} value={target.id}>{target.description} · {target.id}</option>)}</select></label><label className="field"><span>Randomized split</span><select value={splitIndex} onChange={(event) => setSplitIndex(Number(event.target.value))}>{splitOptions.map((split) => <option key={split} value={split}>Split {split + 1}</option>)}</select></label><button onClick={() => setPlayNonce((value) => value + 1)} disabled={!selectedTarget}>Play all displayed</button></div> : null}{message ? <p className="control-hint admin-message">{message}</p> : null}</section>
